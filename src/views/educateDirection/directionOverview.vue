  ...
  <div>
    <div class="liucheng-box">
      <div class="liucheng_list w1080 middle">
        <div class="liucheng_list_item justify" v-for="(e, index) in directionData" :key="index" @click="viewJob(e.subId)">
          <div class="item_intro inline">
            <div class="item_title f20 c3 pointer" @click="viewJob(e.subId)">{{e.subitem}}</div>
            ...
            <div class="readAll float-right clearfix f12 pointer" @click="viewJob(e.subId)">浏览相关岗位</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    this.directionData = this.overView[this.id - 1].children;
  ...
  methods: {
    ...
    viewJob(subId) {
      // 后期要考虑职位未维护的情况
      this.$router.push({path: '/educateDirection/viewJob', query: {subId: subId} })
    }
  }
}
</script>
